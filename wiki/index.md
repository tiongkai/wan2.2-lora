...
- [VLM Models for Auto-Captioning](concepts/captioning-vlms.md) — VLM comparison for sensitive-content captioning. Qwen2.5-VL abliterated recommended. Refusal rates, quality benchmarks, Ollama tags.
...
- [End-to-End Run Findings — 2× RTX A5500 (24GB)](synthesis/dual-a5500-run-findings.md) — First-hand results from the first full fighting-LoRA run: training VRAM fix (blocks_to_swap 30 + expandable_segments), generation VRAM fix (lowvram + disable-cuda-malloc + CLIP on CPU), stock-models decision, loss behavior, and why clips are ~2 s not 5 s. (Jun 2026)
- [Long-Video Generation with Wan 2.2](synthesis/long-video-generation-wan22.md) — Pushing past 5 s: SVI (Stable Video Infinity) error-recycling, SVI 2.0 LoRAs, MoE sigma boundary + WanMoEScheduler auto-shift, FLF2V (WanFirstLastFrameToVideo) extend-and-stitch, Qwen InSubject/InScene keyframe LoRAs. Adversarially verified; flags unverified/refuted. (Jun 2026)
